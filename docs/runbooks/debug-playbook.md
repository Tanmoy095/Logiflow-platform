...
### 2.8 Smoke Test Failure (Local Validation)

**Symptom:** `./scripts/dev/smoke-k8s.sh` exits with error at any step.  
**Trigger (test):** run the smoke test with invalid service name, bad image tag, or wrong probe path.
...
# If script fails, inspect the output step number
# Check pods and events as usual
kubectl get pods -n <namespace> -l app.kubernetes.io/name=<service>
kubectl describe pod <pod-name> -n <namespace>
kubectl get events -n <namespace> --sort-by='.lastTimestamp' | tail -20
...
Docker build failure (Dockerfile missing or build error)

ImagePullBackOff due to wrong tag

Helm lint or template error

Readiness/liveness probe path misconfigured

Missing Helm dependency (run helm dependency update inside chart directory)

Service selector mismatch leading to empty endpoints

Fix: Correct the underlying issue and re-run the smoke test. The script is idempotent—it will rebuild, redeploy, and re-verify.

text

Also update **Prevention Table** (section 5) with a row:

```markdown
| Smoke test failure | `scripts/dev/smoke-k8s.sh` catches build, deploy, and health issues early; run before commit and in CI. |
What git checkout -- <file> Means
The command:

bash
git checkout -- deployment/helm/services/llm-gateway/values-dev.yaml
restores the file values-dev.yaml to the version stored in Git, discarding any uncommitted local changes. The -- signals that the path is a file, not a branch. This is used to undo temporary edits (like changing an image tag to nonexistent or a probe path to /broken) after a failure test.

What the Following Command Does
bash
SERVICE=llm-gateway NAMESPACE=logiflow-dev ./scripts/dev/smoke-k8s.sh
It runs the smoke test for the llm-gateway service in the logiflow-dev namespace. The script will:
...
So together, the two commands reset the values file to the last committed state and then run a full clean verification—ensuring the service works with the correct configuration before you proceed.
